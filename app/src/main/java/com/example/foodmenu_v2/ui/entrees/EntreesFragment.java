...
public class EntreesFragment extends Fragment {
    ...
    private void GetDataFromDb(){
        EntreesListData = new ArrayList<Product>();
        ...
        Query query = databaseRef.orderByChild("category").equalTo("entrees");
        ...
        query.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    EntreesListData.add(newProduct);
                }
                if(EntreesListData.size() == 0)
                    Toast.makeText(getContext(), ":| Nu s-a returnat nici un produs in aceasta categorie! :(", Toast.LENGTH_SHORT).show();

                ProductAdapter adapter = new ProductAdapter(EntreesListData, getContext());
